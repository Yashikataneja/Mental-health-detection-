...
//   // Make setSidebarOpen available globally for navbar
//   useEffect(() => {
...
//     window.addEventListener('toggleChatSidebar', handleSidebarToggle);
    
//     return () => {
//       document.removeEventListener("click", handleClickOutside);
//       window.removeEventListener('toggleChatSidebar', handleSidebarToggle);
//     };
//   }, []);
...
//           history: messages.filter(m => m.sender !== "bot" || messages.indexOf(m) > 0).slice(-10),
//         }),
//       });
...
//       const chatId = data.chatId || activeChatId;
//       if (data.chatId && !activeChatId) {
//         setActiveChatId(data.chatId);
//         setChats((prev) => [{ _id: data.chatId, messages: updatedMessages, createdAt: new Date(), title: data.chatTitle }, ...prev]);
//       } else {
//         setChats((prev) => prev.map((c) => c._id === chatId ? { ...c, messages: updatedMessages } : c));
//       }
...
//       <div className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:relative top-0 md:top-auto left-0 z-50 md:z-auto w-80 md:w-64 h-full md:h-auto flex-shrink-0 flex flex-col overflow-hidden border-r shadow-xl md:shadow-sm transition-transform duration-300 ease-out ${darkMode ? "bg-gray-900 border-gray-700 text-white" : "bg-white border-gray-200 text-slate-800"}`}>
...
//           <button
//             onClick={() => setSidebarOpen(false)}
//             className={`p-2 rounded-lg transition ${darkMode ? "hover:bg-gray-800 text-gray-400" : "hover:bg-gray-100 text-gray-500"}`}
//           >
//             ✕
//           </button>
//         </div>
...
//         {/* New Chat Button */}
//         <div className={`p-3 pt-4 md:pt-20 pb-6 border-b ${darkMode ? "border-gray-700" : "border-gray-200"}`}>
//           <button
//             onClick={() => {
...
//                             e.stopPropagation();
//                             try {
//                               await fetch(`http://localhost:5000/api/chat/${chat._id}`, { method: "DELETE" });
//                               setChats((p) => p.filter((c) => c._id !== chat._id));
...
//                 <div className="w-12 h-12 rounded-2xl bg-gradient-to-r from-indigo-500 via-blue-500 to-cyan-400 flex items-center justify-center shadow-lg">
...
//                       </h3>
//                       <p className={`text-sm ${mutedText}`}>How are you feeling today?</p>
//                     </>
//                   ) : (
//                     <>
//                       <h3 className="font-bold text-lg">
