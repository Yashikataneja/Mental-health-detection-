...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || "admin@mannlytics.com";
...
// DELETE /api/admin/users/:id — delete user + their journals
router.delete("/users/:id", adminMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ message: "User not found" });

    // Delete all journals of this user too
...
// DELETE /api/admin/journals/:id — delete single journal entry
...
router.get("/analytics", adminMiddleware, async (req, res) => {
  try {
    ...
    const riskAgg = await Journal.aggregate([
      {
        $group: {
          ...
          moderateRisk: { $sum: { $cond: [{ $and: [{ $gte: ["$riskScore", 40] }, { $lt: ["$riskScore", 70] }] }, 1, 0] } },
          lowRisk: { $sum: { $cond: [{ $lt: ["$riskScore", 40] }, 1, 0] } },
        },
      },
    ]);
    ...
    res.json({
      totalUsers,
      totalJournals,
      riskStats: riskAgg[0] || { avgRisk: 0, highRisk: 0, moderateRisk: 0, lowRisk: 0 },
      emotionBreakdown: emotionAgg,
      dailyEntries: dailyAgg,
    });
  } catch (err) {
...
// GET /api/admin/library/public — no auth needed, for frontend users
...
// GET /api/admin/safespace/public — no auth needed, for frontend users
